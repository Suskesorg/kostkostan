...
// === SMART DATE PARSER: handles DD-MM-YYYY, DD/MM/YYYY, YYYY-MM-DD ===
...
const fmtD = s => { const d = parseDate(s); if (!d) return "\u2014"; return d.toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }); };
const getDow = (y, m, d) => new Date(y, m, d).getDay();
const inDay = (t, y, m, d) => {
  const j = parseDate(t.dateJoined);
  if (!j) return false;
  const c = new Date(y, m, d);
  const e = parseDate(t.dateEnd) || new Date(2099, 11, 31);
  return c >= j && c <= e;
};
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',system-ui,sans-serif", background: "#0F1117", color: "#E8E9ED", minHeight: "100vh" }}>
      <style>{`
        ...
        .tb:hover{filter:brightness(1.2);transform:scaleY(1.06)}
        *::-webkit-scrollbar{width:6px;height:6px}
        *::-webkit-scrollbar-track{background:#14161E}
      ...
      <div style={{ padding: "20px 24px 0", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
          <div style={{ background: occRate > 70 ? "rgba(91,189,114,0.08)" : occRate > 40 ? "rgba(232,145,58,0.08)" : "rgba(217,74,107,0.08)", border: "1px solid " + (occRate > 70 ? "rgba(91,189,114,0.2)" : occRate > 40 ? "rgba(232,145,58,0.2)" : "rgba(217,74,107,0.2)"), borderRadius: "20px", padding: "6px 14px", fontSize: "13px" }}>
            <span style={{ fontWeight: 600, color: occRate > 70 ? "#5BBD72" : occRate > 40 ? "#E8913A" : "#D94A6B" }}>{occRate}%</span><span style={{ color: "#6B7186", marginLeft: "4px" }}>occupied</span>
          </div>
      ...
      <div style={{ padding: "12px 24px 0", display: "flex", gap: "0", borderBottom: "1px solid #23262F" }}>
        <button className="nav-btn active" style={{ color: "#4A90D9", borderBottomColor: "#4A90D9" }}>📊 Dashboard</button>
        <a href="/reminder" style={{ textDecoration: "none" }}>
          <button className="nav-btn" style={{ position: "relative" }}>
            🔔 Reminder
            {expiringSoon.length > 0 && (
              <span style={{ position: "absolute", top: "4px", right: "4px", background: "#D94A6B", color: "#fff", fontSize: "10px", fontWeight: 700, borderRadius: "50%", width: "18px", height: "18px", display: "flex", alignItems: "center", justifyContent: "center" }}>{expiringSoon.length}</span>
            )}
          </button>
        </a>
      </div>

      {error && <div style={{ margin: "12px 24px", padding: "12px 16px", background: "rgba(217,74,107,0.1)", border: "1px solid rgba(217,74,107,0.2)", borderRadius: "8px", color: "#D94A6B", fontSize: "13px" }}>{error}</div>}
      ...
      <div style={{ padding: "0 24px 24px" }}>
        <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 280px)", borderRadius: "12px", border: "1px solid #23262F", background: "#14161E" }}>
      ...
      {modal && sel && <div onClick={() => setModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "20px", animation: "fadeIn 0.2s" }}>
        <div onClick={e => e.stopPropagation()} style={{ background: "#1A1D27", borderRadius: "16px", padding: "28px", width: "100%", maxWidth: "380px", border: "1px solid #2A2D37", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", animation: "slideUp 0.25s" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            {sel.dateEnd && <InfoRow icon="🚪" label="End Date" value={fmtD(sel.dateEnd)} />}
            {sel.dateJoined && (() => {
              const j = parseDate(sel.dateJoined);
              if (!j) return null;
              const df = now - j, mo = Math.floor(df / (1000 * 60 * 60 * 24 * 30.44)), dy = Math.floor((df % (1000 * 60 * 60 * 24 * 30.44)) / (1000 * 60 * 60 * 24));
              return <InfoRow icon="⏱️" label="Duration" value={mo + " bulan, " + dy + " hari"} />;
            })()}
